...
int val;
int cnt;
volatile uint32_t radar_adc_val = 0;
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
//  // 1. FFT 초기화
//    // 초기화를 안 하면 S 구조체가 비어있어서 FFT 함수가 멈춥니다.
...
//
//	// 3. ADC DMA 시작 (이제부터 자동으로 adc_buffer에 값이 채워짐)
//	  if (HAL_ADC_Start_DMA(&hadc1, (uint32_t*)adc_buffer, FFT_LEN) != HAL_OK) {
